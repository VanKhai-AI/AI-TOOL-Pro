...
const EdutainmentOutlineGenerator: React.FC = () => {
    ...
    const getPrompt = () => {
      ...
      return `{
  "role": "You are an Edutainment Content Architect and Expert Science Communicator. Your mission is to create a detailed, logically structured, and highly engaging video outline for the channel '${channelName}', adhering to the best practices of visual explanation and simplifying complex topics.",
  "input_information": {
    "output_language": "${language}",
    "video_topic": "${documentaryTopic}",
    "user_ideas_main_message": "${mainMessage}",
    "teaching_style_reference": "A '${teachingStyle}' style that simplifies complex topics through clear narration, clever analogies, and strong visual storytelling (infographics, animations). The tone should be curious, enthusiastic, and accessible, making learning feel like an exciting discovery.",
    "desired_video_duration": "${duration}",
    "reading_speed": "${readingSpeed}",
    "target_total_words": "${totalWords}"
  },
  "internal_process_for_best_results": {
    "1_hook_analysis_and_ideation": "Brainstorm a powerful 'hook' based on the topic. It must be a surprising fact, a counter-intuitive question, or a relatable problem that immediately draws the viewer into the core question of the video. The hook must promise a clear and satisfying explanation.",
    "2_word_allocation_to_5_part_structure": "Allocate the total target words (${totalWords}) into a 5-part structure designed for maximum clarity and retention:\\n- **Part 1: Giới Thiệu & Hook (Introduction & Hook):** ~10% of words. Start with the compelling hook. Clearly state the central question the video will answer. Briefly outline what the viewer will learn.\\n- **Part 2: Bối Cảnh & Nền tảng (Context & Foundation):** ~20% of words. Explain the essential, foundational concepts needed to understand the main topic. Use simple analogies. This is the 'Before we get to X, you need to understand Y' section.\\n- **Part 3: Giải Thích Cốt Lõi (Core Explanation / Deep Dive):** ~45% of words. This is the main event. Break down the core topic step-by-step. Describe the key mechanisms, processes, or ideas. This section should be rich with descriptions that suggest visual aids (animations, infographics).\\n- **Part 4: Ý Nghĩa & Ứng Dụng (Implications & Real-World Connection):** ~20% of words. Connect the core topic to the real world. Why does this matter? What are the practical applications, future implications, or surprising connections to everyday life?\\n- **Part 5: Kết Luận & Kêu Gọi Hành Động (Conclusion & CTA):** ~5% of words. Briefly summarize the main answer/takeaway in a memorable way. Hint at related future topics. End with a clear call to action (subscribe, like, comment with questions)."
  },
  "output_outline_rules": {
    "1_clear_part_naming": "The title of each part must be clear, descriptive, and curiosity-driven. The first part's title should contain 'Giới Thiệu' or 'Introduction', and the last should contain 'Kết Luận' or 'Conclusion'.",
    "2_paragraph_calculation": "For each part you create, calculate \`estimatedParagraphs\` by taking \`estimatedWords\` and dividing by ${paragraphWordCountAvg}, rounding to the nearest integer.",
    "3_language_and_tone": "The entire outline MUST be written in the **${language}** language. The tone should be curious, clear, enthusiastic, and accessible, perfect for an edutainment channel."
  },
  "json_output_format_required": "- You MUST return a single JSON object.\\n- The object must contain a 'title' key with the value '${documentaryTopic}'.\\n- It must also contain a 'parts' key, which is an array of part objects.\\n- Each object in the 'parts' array must have this structure:\\n    - **part**: Sequential number starting from 1.\\n    - **title**: Descriptive title for the part (e.g., 'Phần 1: Lỗ đen là gì?').\\n    - **description**: A brief summary of the content and the part's role in explaining the topic.\\n    - **estimatedWords**: Estimated word count for this part.\\n    - **estimatedParagraphs**: Estimated paragraph count for this part."
}`;
    };
