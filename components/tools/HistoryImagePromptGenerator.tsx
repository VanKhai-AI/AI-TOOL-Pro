...
`;
    ...
    const handleGenerate = useCallback(async () => {
        if (!scriptText) {
            updateToolState('history-image-prompts', { error: "Kịch bản trống." });
            return;
        }
        updateToolState('history-image-prompts', { isLoading: true, error: null, generatedPrompts: [] });
        setProgress(0);
        ...
        for (let i = 0; i < totalSentences; i++) {
            try {
                ...
                if (result && result.imagePrompt) {
                    ...
                    updateToolState('history-image-prompts', { generatedPrompts: [...newPrompts] });
                ...
                updateToolState('history-image-prompts', { error: `Đã xảy ra lỗi ở câu ${i + 1}: ${e.message}. Quá trình đã dừng lại.` });
                updateToolState('history-image-prompts', { isLoading: false });
                return;
            }
        }
    ...
    return (
        <ToolLayout tool={tool} language={'vi'} setLanguage={() => {}} getPrompt={() => getPromptForSentence('[Câu kịch bản mẫu]', true)}>
            <div className="flex flex-col gap-6">
                <Textarea label="Kịch bản từ Bước 3" id="script" value={scriptText} onChange={e => updateToolState('history-image-prompts', { scriptText: e.target.value })} placeholder="Dán toàn bộ kịch bản đã tải về từ Bước 3..." rows={12} required />
...
export default HistoryScriptImagePromptGenerator;
